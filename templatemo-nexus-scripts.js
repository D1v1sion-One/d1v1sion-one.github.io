...
fileInput.addEventListener("change", async () => {
  ...
  output.textContent = "Scanning...🔎︎";

  try {
    const response = await fetch("https://vudraapi.onrender.com/scans/single", {
      method: "POST",
      body: formData,
    });
      ...
      output.innerHTML = `
        ...
        ${result.findings
          .map(
            (finding) => `
          <div class="finding">
            <strong>Title:</strong> ${finding.title}<br><br>
            <p></p>
            <strong>Severity:</strong> ${finding.severity}<br><br>
            <p></p>
            <strong>Message:</strong> ${finding.message}<br><br>
            <p></p>
            <strong>Line:</strong> ${finding.line_number}<br><br>
          </div>
          <hr>
        `,
          )
    ...
    output.textContent = err.message + "...Need Internet";
    console.error("Error: ", err);
  }
});

function scrollToTheButtom() {
